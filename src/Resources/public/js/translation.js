const SyliusTranslationPlugin = {
            ...
            modalContainer
                ...
                .modal({
                    onApprove: () => {
                        ...
                        const translationIdInput = document.getElementById(
                            "addDomainMessageModal_translationIdInput"
                        );
                        const translationInput = document.getElementById(
                            "addDomainMessageModal_translationInput"
                        );
                        if (
                            translationIdInput.value.length > 0 &&
                            translationInput.value.length > 0
                        ) {
                            this.setMessage(
                                localeCode,
                                domain,
                                translationIdInput.value,
    ...
    filterTranslations(filters) {
        ...
        Array.from(translationRows).forEach((translationElement) => {
            ...
            if (!applyFilters.isTranslated && isTranslated)
                translationElement.classList.add("hidden");
            if (isCustom) {
                if (applyFilters.isCustom && isCustom)
                    translationElement.classList.remove("hidden");
            }
            if (!applyFilters.isUntranslated && !isTranslated)
                translationElement.classList.add("hidden");
...
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll(".progress").forEach((el) => {
        el.classList.add("initialized-progress");
    });
    document.querySelectorAll(".translation_input").forEach((input) => {
    ...
    if (filterDomainElement)
        filterDomainElement.onchange = (event) => SyliusTranslationPlugin.applyFilters();
    if (filterIdElement)
        filterIdElement.onchange = (event) => SyliusTranslationPlugin.applyFilters();
    if (filterTranslatedElement)
        filterTranslatedElement.onchange = (event) => SyliusTranslationPlugin.applyFilters();
    if (filterUntranslatedElement)
        filterUntranslatedElement.onchange = (event) => SyliusTranslationPlugin.applyFilters();
    if (filterCustomElement)
        filterCustomElement.onchange = (event) => SyliusTranslationPlugin.applyFilters();
    if (
        filterDomainElement &&
        filterIdElement &&
...
window.uiAlert = function(options) {
    ...
    let UiAlertMessagesContainer = document.getElementsByClassName(
        `ui-alert-content-${setUI.position}`
    )[0];
